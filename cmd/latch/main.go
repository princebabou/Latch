...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	case "conformance":
		return conformanceCommand(args[1:], out, errOut)
	case "playground":
		return playgroundCommand(args[1:], out, errOut)
	case "run":
	...
	case "proxy-api":
		return proxyAPI(args[1:], out, errOut)
	case "serve":
...
Usage:
  ...
  latch conformance [--json|--manifest]
...
The check and run commands only evaluate requests. Every proxy forwards only
actions that Latch explicitly allows.
`)
}
